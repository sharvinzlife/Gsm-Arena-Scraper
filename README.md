...
- 🖼️ **High-Resolution Images**:
  ...
  - Ability to embed images directly into JSON files as base64
...
#### Installing Java JDK 17+ (OpenJDK)
...
chmod +x run.sh run-batch.sh run-menu.sh embed-images.sh recover-images.sh
```
...
The project includes several convenience scripts to make execution easier:

```bash
# Run the main menu interface (recommended for beginners)
./run-menu.sh

# Run the interactive scraper directly
./run.sh

# Run the batch scraper directly
./run-batch.sh

# Embed images into JSON files after scraping
./embed-images.sh final_data/phones
# OR
./embed-images.sh interactive_data/phones

# Recover and embed missing images (useful if you have issues with image embedding)
./recover-images.sh
```

### Interactive Mode

The interactive mode allows you to manually search for specific phone models:
